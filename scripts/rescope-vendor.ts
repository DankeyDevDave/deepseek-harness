...
const GENERIC_SKIPS: readonly GenericSkip[] = [
  ...
  // The dynamic Cordis runtime forwards its events on the `cordis/` scope —
  // `'cordis/request-run'` and siblings are wire channel names emitted by the
  // Loader's `cordis:` builtin, so the scoped package name would no longer
  // match what the Host emits — and its UI/i18n namespace key is the bare
  // `cordis` string. In these files the name is a runtime identifier, not a
  // package reference.
